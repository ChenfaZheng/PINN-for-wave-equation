...
class DataGenerator():
    ...
    def gen_bound(self, N_bound):
        ...
        xx0 = tf.reshape(xx0, (N_bound**3, 1))
        yy0 = tf.reshape(yy0, (N_bound**3, 1))
        zz0 = tf.reshape(zz0, (N_bound**3, 1))
        tt0 = tf.zeros_like(xx0) + self.tmin

        X0 = tf.concat([tt0, xx0, yy0, zz0], 1)
    ...
    def gen_predict(self, N_predict, t=None):
        ...
        z = tf.reshape(tf.cast(tf.linspace(self.zmin, self.zmax, N_predict), dtype=DTYPE), (N_predict, 1))
        xx, yy, zz = tf.meshgrid(x, y, z)
        xx = tf.reshape(xx, (N_predict**3, 1))
        yy = tf.reshape(yy, (N_predict**3, 1))
        zz = tf.reshape(zz, (N_predict**3, 1))
        if t is None:
            t_view = self.tmax
        else:
            t_view = t
    ...
    def gen_predict_planes(self, N_predict, t=None):
        x = tf.reshape(tf.cast(tf.linspace(self.xmin, self.xmax, N_predict), dtype=DTYPE), (N_predict, 1))
        y = tf.reshape(tf.cast(tf.linspace(self.ymin, self.ymax, N_predict), dtype=DTYPE), (N_predict, 1))
        z = tf.reshape(tf.cast(tf.linspace(self.zmin, self.zmax, N_predict), dtype=DTYPE), (N_predict, 1))
        xy, yx = tf.meshgrid(x, y)
        xz, zx = tf.meshgrid(x, z)
        yz, zy = tf.meshgrid(y, z)
        xym, yxm = tf.reshape(xy, (N_predict**2, 1)), tf.reshape(yx, (N_predict**2, 1))
        xzm, zxm = tf.reshape(xz, (N_predict**2, 1)), tf.reshape(zx, (N_predict**2, 1))
        yzm, zym = tf.reshape(yz, (N_predict**2, 1)), tf.reshape(zy, (N_predict**2, 1))
        if t is None:
            t_view = self.tmax
        else:
            t_view = t
        tt = tf.ones_like(xym) * t_view
        zerosm = tf.zeros_like(xym)
        Xz0 = tf.concat([tt, xym, yxm, zerosm], 1)
        Xx0 = tf.concat([tt, zerosm, yzm, zym], 1)
        Xy0 = tf.concat([tt, xzm, zerosm, zxm], 1)
        return Xx0, Xy0, Xz0
...
class DataModel():
    ...
    def u_forward(self, X):
        ...
        u = tf.exp(-(x - self.a*t)**2 - (y - self.a*t)**2 - (z - self.a*t)**2)
        return u * 0.5
...
class Plotter():
    ...
    def plot_sample(self, X0, u0, ut0, X_train):
        ...
        ax.set_ylabel('$y$')
        ax.set_zlabel('$z$')
        ax.set_title('Positions of training samples')
        # ax.legend()
    ...
    def plot_animated_contour(self, ts, X_tests, u_predicts, N_test, title='Predict', plane='xy'):
        ...
        if plane == 'xy' or plane == 'yx':
            ...
            raise ValueError('plane must be xy, yx, xz, zx, yz, or zy')
        fig.colorbar(cax, ax=ax, fraction=0.1, pad=0.1)
...
class WaveModel(tf.keras.Sequential):
    ...
    def compute_loss(self, X0, u0, ut0, Xb, X_train, w_u0, w_ut0, w_ub, X0_val, u0_val, ut0_val, Xb_val):
        ...
        ubz_pred = tape.batch_jacobian(ub_pred, Xb)[:, :, 3]
        ubz_pred_val = tape.batch_jacobian(ub_pred_val, Xb_val)[:, :, 3]
        l3 = tf.reduce_mean(tf.square(ubx_pred) + tf.square(uby_pred) + tf.square(ubz_pred))
        l3_val = tf.reduce_mean(tf.square(ubx_pred_val) + tf.square(uby_pred_val) + tf.square(ubz_pred_val))
...
def run(config, work_dir):
    ...
    Xx_tests, Xy_tests, Xz_tests = [], [], []
    ux_predicts, uy_predicts, uz_predicts = [], [], []

    for i, t_view in enumerate(ts):
        X_test = wave_datagen.gen_predict(N_test, t=t_view)
        Xx_test, Xy_test, Xz_test = wave_datagen.gen_predict_planes(N_test, t=t_view)
        ...
        Xx_tests.append(tf.concat([Xx_test[:, 0:1], Xx_test[:, 2:3], Xx_test[:, 3:4]], axis=1))
        Xy_tests.append(tf.concat([Xy_test[:, 0:1], Xy_test[:, 1:2], Xy_test[:, 3:4]], axis=1))
        Xz_tests.append(tf.concat([Xz_test[:, 0:1], Xz_test[:, 1:2], Xz_test[:, 2:3]], axis=1))
        ux_predicts.append(ux_predict)
        uy_predicts.append(uy_predict)
        uz_predicts.append(uz_predict)
    
    print('saving animated gif...', flush=True)
    plotter.plot_animated_contour(ts, Xx_tests, ux_predicts, N_test, 'Prediction_yz', plane='yz')
    plotter.plot_animated_contour(ts, Xy_tests, uy_predicts, N_test, 'Prediction_xz', plane='xz')
    plotter.plot_animated_contour(ts, Xz_tests, uz_predicts, N_test, 'Prediction_xy', plane='xy')
...
def main():
    ...
    # save
    with open(os.path.join(work_dir, 'errs-gaussian-3d-reflection2-{}-{}.csv'.format(config['train']['N_iter'], config['boundary']['relative_noise'])), 'w') as f:
