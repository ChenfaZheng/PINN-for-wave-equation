...
class WaveModel(tf.keras.Sequential):
    ...
    def compute_loss(self, X0, u0, ut0, Xb, X_train, w_u0, w_ut0, w_ub, X0_val, u0_val, ut0_val, Xb_val):
        ...
        # loss set as u_x(t, x=xmax, y) == 0 and u_y(t, x, y=ymax) == 0
        with tf.GradientTape(persistent=True) as tape:
            tape.watch(Xb)
            tape.watch(Xb_val)
...
def main():
    ...
    # save
    with open(os.path.join(work_dir, 'errs-sephere-2d-reflection2-{}-{}.csv'.format(config['train']['N_iter'], config['boundary']['relative_noise'])), 'w') as f:
