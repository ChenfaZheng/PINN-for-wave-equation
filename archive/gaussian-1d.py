...
class DataGenerator():
    def __init__(self, model, tmin, tmax, xmin, xmax, relative_noise=0.1):
        self.model = model
        self.tmin = tmin
        self.tmax = tmax
        self.xmin = xmin
    ...
    def get_lb_ub(self):
        lb = tf.constant([self.tmin, self.xmin], dtype=DTYPE)
        ub = tf.constant([self.tmax, self.xmax], dtype=DTYPE)
        return lb, ub
    ...
    def gen_bound(self, N_bound):
        ...
        t0 = tf.ones_like(x0) * self.tmin
        X0 = tf.concat([t0, x0], 1)

        u0 = self.model.u0(X0) * (1.0 + self._gen_relative_noise(N_bound))
        ut0 = self.model.ut0(X0) * (1.0 + self._gen_relative_noise(N_bound))
    ...
    def gen_bound_t(self, N_bound):
        ...
        # N1 = N_bound // 2
        # X0min = tf.zeros_like(t0[:N1, :]) + self.xmin
        # X0max = tf.zeros_like(t0[N1:, :]) + self.xmax
        # x0 = tf.concat([X0min, X0max], axis=0)
        x0 = tf.zeros((N_bound, 1))
        X0 = tf.concat([t0, x0], 1)
    ...
    def gen_train(self, N_train):
        ...
        X_train = tf.concat([t_train, x_train], 1)
        return X_train
...
class DataModel():
    ...
    def ut0(self, X):
        x = X[:, 1:2]
        ut0 = 2*self.a*x * tf.exp(-tf.pow(x, 2))
        # return ut0
        return tf.zeros_like(ut0)
...
def main():
    ...
    # save
    with open(os.path.join(work_dir, 'errs-gaussian-1d-reflection2-{}-{}.csv'.format(config['train']['N_iter'], config['boundary']['relative_noise'])), 'w') as f:
