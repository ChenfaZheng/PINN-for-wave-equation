...
class DataGenerator():
    ...
    def gen_bound_t(self, N_bound):
        ...
        N1 = N_bound // 2
        X0min = tf.zeros((N_bound, 1), dtype=DTYPE) + self.xmin
        X0max = tf.zeros((N_bound, 1), dtype=DTYPE) + self.xmax
        Y0min = tf.zeros((N_bound, 1), dtype=DTYPE) + self.ymin
        Y0max = tf.zeros((N_bound, 1), dtype=DTYPE) + self.ymax
        X0lin = tf.reshape(tf.linspace(self.xmin, self.xmax, N_bound), (N_bound, 1))
        Y0lin = tf.reshape(tf.linspace(self.ymin, self.ymax, N_bound), (N_bound, 1))

        XXmin = tf.concat([X0min, Y0lin], 1)
        XXmax = tf.concat([X0max, Y0lin], 1)
        YYmin = tf.concat([X0lin, Y0min], 1)
        YYmax = tf.concat([X0lin, Y0max], 1)

        XY = tf.concat([XXmin, XXmax, YYmin, YYmax], 0)
        TT = tf.random.uniform((N_bound*4, 1), minval=self.tmin, maxval=self.tmax, dtype=DTYPE)
        X0 = tf.concat([TT, XY], 1)
...
class DataModel():
    ...
    def u_model(self, X):
        ...
        x = X[:, 1:2]
        y = X[:, 2:3]
        r_numpy = tf.sqrt(x**2 + y**2).numpy()
        r_numpy[r_numpy < 1] = 1
        r = tf.cast(r_numpy, dtype=DTYPE)

        u = tf.sin(t - 1 / self.a * r) / r
        return u
...
class WaveModel(tf.keras.Sequential):
    ...
    def compute_loss(self, X0, u0, ut0, Xb, X_train, w_u0, w_ut0, w_ub, X0_val, u0_val, ut0_val, Xb_val):
        ...
        # loss set as u(t, x=xmax, y) == 0
        ux_pred = self.call(Xb)
        ux_pred_val = self.call(Xb_val)
        l3 = tf.reduce_mean(tf.square(ux_pred))
        l3_val = tf.reduce_mean(tf.square(ux_pred_val))
...
def main():
    ...
    # save
    with open(os.path.join(work_dir, 'errs-sephere-2d-reflection-{}-{}.csv'.format(config['train']['N_iter'], config['boundary']['relative_noise'])), 'w') as f:
